...
def eplanet(request):
    planets = Exoplanets.objects.all()
    specific_time1 = time(22, 0, 0)
    specific_time2 = time(8, 0, 0)
    current_time = datetime.now().time()

    if current_time > specific_time1 or current_time < specific_time2:
        return render(request, 'eplanet/eplanet.html', {"planets": planets, "time": 0})
    else:
        return render(request, 'eplanet/eplanet.html', {"planets": planets, "time": 1})
